...
## 1. Project Overview
- **Vision:** Deterministic, auditable computation of validator revenue and partner commissions across multiple blockchain networks with provider-agnostic architecture
- **Current Phase:** Initial design and architecture planning (MVP targeting Solana, M1 adding Ethereum)
- **Key Architecture:** Chain-agnostic ingestion → normalization → canonical data layer → commission engine → RBAC-enforced API
- **Development Strategy:** On-premise first, Python-first stack, provider swappability, deterministic calculations with full audit trail
